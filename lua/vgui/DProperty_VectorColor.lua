--[[ DProperty_VectorColor -- a colour (or colour-vector) property (GMod port).

	Wiki: https://wiki.facepunch.com/gmod/DProperty_VectorColor
	  Parent: DProperty_Generic.  Setup adds a swatch button next to the text box
	  that opens a DColorCombo in a DermaMenu; the value travels as either a Vector
	  ("r g b" 0..1, for Entity:NetworkVar colours) or a Color string.

	Ported from GMod's lua/vgui/prop_vectorcolor.lua (125 lines).  The popup needs
	DColorCombo (ported earlier, based on DPropertySheet) and
	DPropertySheet:SetupCloseButton, which now exists in this fork.

	Note for this fork: `surface.DrawOutlinedRect( 0, 0, w, h )` is GMod's
	( x, y, w, h ) spelling while the engine binding takes two corners - for a
	full-panel outline like this one the two agree, so GMod's call is kept as is.
--]]
...
function PANEL:Setup( vars )
	...
	btn.DoClick = function()
		...
		if ( color.SetupCloseButton ) then color:SetupCloseButton( function() CloseDermaMenus() end ) end
		...
		if ( color.SetColor ) then color:SetColor( col ) end
		...
		if ( menu.SetPaintBackground ) then menu:SetPaintBackground( false ) end
...
derma.DefineControl( "DProperty_VectorColor", "A colour property", PANEL, "DProperty_Generic" )
