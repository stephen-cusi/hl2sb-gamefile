...
if CLIENT then
language.Add( "npc_scp_049-2", "SCP-049-2" )
end

ENT.Base             = "scp0492base"
ENT.Spawnable        = false
...
function ENT:Reanimate( dmginfo )
	...
	hitgroup = tr.HitGroup
		...
		if hitgroup == HITGROUP_HEAD then
			...
			if math.random(1,5) == 1 then
				if self:Health() < 30 and !dmginfo:IsExplosionDamage() then
					self:Reanimate( dmginfo )
				end	
			end
