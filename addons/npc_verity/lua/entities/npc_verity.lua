...
ENT.JumpSound = Sound("npc_verity/jump.mp3")
ENT.TauntSounds = {
	Sound("npc_verity/pieceofcake.mp3"),
}
local chaseMusic = Sound("npc_verity/panic.mp3")
...
local npc_verity_acquire_distance =
	CreateConVar("npc_verity_acquire_distance", 100000, FCVAR_NONE,
	"The maximum distance at which verity will chase a target.")

local npc_verity_spawn_protect =
	CreateConVar("npc_verity_spawn_protect", 0, FCVAR_NONE,
	"If set to 1, verity will not target players or hide within 200 units of \z
	a spawn point.")

local npc_verity_attack_distance =
	CreateConVar("npc_verity_attack_distance", 80, FCVAR_NONE,
	"The reach of verity's attack.")

local npc_verity_attack_interval =
	CreateConVar("npc_verity_attack_interval", 0.2, FCVAR_NONE,
	"The delay between verity's attacks.")

local npc_verity_attack_force =
	CreateConVar("npc_verity_attack_force", 800, FCVAR_NONE,
	"The physical force of verity's attack. Higher values throw things \z
	farther.")

local npc_verity_smash_props =
	CreateConVar("npc_verity_smash_props", 1, FCVAR_NONE,
	"If set to 1, verity will punch through any props placed in their way.")

local npc_verity_allow_jump =
	CreateConVar("npc_verity_allow_jump", 1, FCVAR_NONE,
	"If set to 1, verity will be able to jump.")

local npc_verity_hiding_scan_interval =
	CreateConVar("npc_verity_hiding_scan_interval", 3, FCVAR_NONE,
	"verity will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant veritys are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_verity_hiding_repath_interval =
	CreateConVar("npc_verity_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to verity's hiding spot will be redetermined every X seconds.")

local npc_verity_chase_repath_interval =
	CreateConVar("npc_verity_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of verity's target will be redetermined every \z
	X seconds.")

local npc_verity_expensive_scan_interval =
	CreateConVar("npc_verity_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_verity_force_download =
	CreateConVar("npc_verity_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download verity resources \z
...
if npc_verity_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("verity_nag")
util.AddNetworkString("verity_navgen")
...
local VECTOR_verity_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_verity_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_verity: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other veritys, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_verity"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "verityMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_verity" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why verity isn't working.
	net.Start("verity_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_verity: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_verity: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_verity: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_verity: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_verity: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, verity in pairs(ents.FindByClass("npc_verity")) do
		verity:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "verityNavGen", navEndGenerate)
	else
		print("npc_verity: nav_generate failed to initialize")
...
concommand.Add("npc_verity_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_verity: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. verity doesn't use
	...
	elseif isConsole then
		print("npc_verity: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_verity_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_verity_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_verity_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_verity_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. verity means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_verity_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_verity_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_verity_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_verity_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_verity_attack_distance:GetInt()
		...
		local repathInterval = npc_verity_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_verity_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_verity_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_verity_hiding_repath_interval:GetFloat()
...
local MAT_verity = Material("npc_verity/verity")
killicon.Add("npc_verity", "npc_verity/killicon", color_white)
language.Add("npc_verity", "verity ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_verity: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a verity.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_verity_music_volume =
	CreateConVar("npc_verity_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by verity. (0-1, where 0 is muted)")

-- If another verity comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, veritys do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_verity_PANIC_COUNT veritys are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 1000

 -- That's a lot of verity.
local MUSIC_verity_PANIC_COUNT = 8

local MUSIC_verity_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_verity_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_verity") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("verityPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_verity_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_verity" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_verity_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear verity taunt us.
...
local function startTimer()
	if not timer.Exists("verityPanicMusicUpdate") then
		timer.Create("verityPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_verity)

	-- Get the normal vector from verity to the player's eyes, and then compute
	...
	-- verity should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("verityHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("verityHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("verity is studying this map.", "verityHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "verityHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "verityHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "verityHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "verityHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("verity_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "verityNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. verity doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_verity_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for verity to figure \z
		this map out.\n\z
...
net.Receive("verity_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! verity doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet verity-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! verity doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to verity.\n\z
			\n\z
			If you ARE the server host, you can run npc_verity_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not verity-compatible!",
			"Ok", nil,
...
--
list.Set("NPC", "npc_verity", {
	Name = "verity",
	Class = "npc_verity",
	Category = "Cool Nextbots Pack",
	AdminOnly = flase
})
